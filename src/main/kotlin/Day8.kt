...
class Day8 {
    fun countObviousDigits(entries: List<Entry>): Int {
        return entries.flatMap { it.outputs }.count { setOf(2, 3, 4, 7).contains(it.length) }
    }
}
...
gba eagbcf gb gbacf dacgebf bceg gcafed acfeg acbfd adegfb | abfcdeg dbcfage faecg afebgc""".split("\n").map {
    val (inputs, outputs) = it.split(" | ")
    Entry(inputs.split(" "), outputs.split(" "))
}

data class Entry(val inputs: List<String>, val outputs: List<String>)
